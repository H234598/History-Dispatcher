...
APPLY_CONFIRMATION = "MIGRATE-CREDENTIALS-V4"


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Preflight, dry-run, apply, or verify the additive native Telegram "
            "credential metadata schema-v4 migration."
        )
    )
    ...
    *,
    ...
    try:
        ...
        if args.command == "verify":
            report = verify_database_v4(database_path)
            _print(report)
            return 0 if report["ok"] else 1

        provider = key_provider or SecretServiceKeyProvider()
        migrator = DatabaseV4Migrator(
            database_path,
            provider,
    ...
    except (MigrationV4Error, KeyUnavailable, OSError, ValueError) as exc:
        _print(_error("credential_schema_migration_failed", exc))
        return 1
