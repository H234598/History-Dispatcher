...
| `SI-004` | Ein fehlender oder ungültiger Schlüssel führt niemals zu Klartextfallback. | `KeyUnavailable`; keine alternative Plaintextspeicherung | `test_payload_crypto_is_aes_gcm_with_secret_service_and_no_plaintext_fallback` |
...
| `SI-006` | Das Cinnamon-Applet greift weder direkt auf SQLite noch auf Telegram, HTTP oder Vault zu. | Snapshotreader und fester CLI-Einstieg | `tests/applet_contract.test.js` (allowlisteter fester argv-Einstieg) und bestehende Snapshot-Statiktests |
| `SI-007` | Applet-Subprozesse werden nicht über Shellstrings gestartet; erlaubte Aktionen sind explizit allowlistet. | direkte argv, `ALLOWED_ACTIONS`, CLI-`applet-action` | Node-Appletvertragstest und bestehende Applet-Statiktests |
...
| `SI-009` | Dienst und Collector laufen mit gehärteten systemd-User-Units. | `NoNewPrivileges`, `ProtectSystem`, `ProtectHome`, `RestrictNamespaces`, `UMask` | `test_systemd_units_keep_local_only_hardening_baseline` und bestehende Unit-Tests |
| `SI-010` | Appletfehler oder -entfernung stoppen weder Dienst, Collector noch Queue. | Removal-Hook räumt nur lokale Cancellables, Timer und Menü auf | `tests/applet_contract.test.js` (Removal ohne Backendspawn) und isolierter Cinnamon-Lauf |
