...
**Stand:** abgeschlossener Provider-v2-Cutover / Vorbereitung Config-v2-Writer  
...
| `REUSE-008` | gleicher TeeBotus-Stand | `dispatch_codex_history_outbox`, `_dispatch_codex_history_outbox_via_dispatcher`, Callback-Spool | Muster für Claim, Partial Results, Erfolgsausschluss und Reconciliation | wie `REUSE-006`; lokal neu formuliert | Provider-v2- und TeeBotus-Cutovertests |
...
| `REUSE-011` | `H234598/TeeBotus@36c75843a5910cc3b22ffdd9a5ec87eb1d5b2ea9` | expliziter `provider_v2`-Modus, fail-closed Batchworker, private Route vor Claim, `possible_duplicate`-Sendverbot | gemergter produktiver Cutover als Referenz für nativen Provider und gemeinsamen Fault-Korpus | wie `REUSE-006` | Core-, Benchmark-, Plan2-, Rebind- und Cutovertests |
| `REUSE-012` | gleicher TeeBotus-Cutover | verschlüsselter Callback-Rebind, Target-Reclaim, atomare Umschreibung und `reconciliation_only`-Sendgrenze | Cross-Repository-Crash-after-Accept-Härtung; Serververtrag im History-Dispatcher eigenständig implementiert | wie `REUSE-006` | `tests/test_provider_api_v2_reclaim.py`, TeeBotus-Rebind- und Boundarytests |
...
| `H234598/TeeBotus@36c7584…` | Bot-API-Transport, Formatter, Rate-Limit, Attachment- und gruppierte Menüs | Herkunft fixiert; Root-Lizenz vor größerer wörtlicher Übernahme ergänzen oder Verhalten lokal neu formulieren |
...
- Raw Codex-Rollouts, Bot-Tokens, Chat-IDs, rohe Message-Refs, private
  Account-IDs und absolute Pfade werden niemals als Fixture oder
  Dokumentationsbeispiel übernommen.
...
- `reconciliation_only` wird serverseitig markiert sowie im TeeBotus-
  Transportadapter und Sendworker vor jeder Registrierung oder Sendoperation
  abgewiesen.
