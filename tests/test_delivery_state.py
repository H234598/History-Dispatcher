...
def test_target_success_is_monotonic_and_retry_path_is_explicit() -> None:
    ...
    assert (
        ensure_target_transition("claimed", "failed_retryable")
        is TargetDeliveryState.FAILED_RETRYABLE
    )
    assert (
        ensure_target_transition("failed_retryable", "pending")
        is TargetDeliveryState.PENDING
    )
...
def test_recipient_success_rank_cannot_be_downgraded_or_retried() -> None:
    assert (
        ensure_recipient_transition("pending", "accepted")
        is RecipientDeliveryState.ACCEPTED
    )
    assert (
        ensure_recipient_transition("accepted", "delivered")
        is RecipientDeliveryState.DELIVERED
    )
    assert (
        ensure_recipient_transition("delivered", "acknowledged")
        is RecipientDeliveryState.ACKNOWLEDGED
    )
    for state in ("accepted", "delivered", "acknowledged"):
        assert recipient_state_is_successful(state) is True
        assert recipient_state_is_terminal(state) is True
    with pytest.raises(InvalidDeliveryTransition):
...
def test_possible_duplicate_requires_reconciliation() -> None:
    assert (
        ensure_recipient_transition("claimed", "possible_duplicate")
        is RecipientDeliveryState.POSSIBLE_DUPLICATE
    )
    assert (
        ensure_recipient_transition("possible_duplicate", "delivered")
        is RecipientDeliveryState.DELIVERED
    )
